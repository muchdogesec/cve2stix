...
def test_main_runs_chord_flow(celery_always_eager, config):
    with (
        ...
        start_date, end_date = datetime(2025, 7, 1, 0, 0, tzinfo=UTC), datetime(2025, 7, 12, 0, 0, tzinfo=UTC)
        # Act
...
def test_fetch_data(config):
    with patch('cve2stix.main.fetch_url') as mock_fetch_url:
        ...
        mock_fetch_url.assert_called_once_with('https://services.nvd.nist.gov/rest/json/cves/2.0/?lastModStartDate=2025-07-01T00%3A00%3A00Z&lastModEndDate=2025-08-01T00%3A00%3A00Z', config, main.parse_cve_api_response)
